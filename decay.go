...
func (d *decayer) RegisterDecayingTag(name string, interval time.Duration, decayFn connmgr.DecayFn, bumpFn connmgr.BumpFn) (connmgr.DecayingTag, error) {
	...
	if interval < d.cfg.Resolution {
		d.logger.Warn(fmt.Sprintf("decay interval for %s (%s) was lower than tracker's resolution (%s); overridden to resolution",
			name, interval, d.cfg.Resolution))
		interval = d.cfg.Resolution
	}

	if interval%d.cfg.Resolution != 0 {
		d.logger.Warn(fmt.Sprintf("decay interval for tag %s (%s) is not a multiple of tracker's resolution (%s); "+
...
func (t *decayingTag) Close() error {
	if !atomic.CompareAndSwapInt32(&t.closed, 0, 1) {
		t.trkr.logger.Warn(fmt.Sprintf("duplicate decaying tag closure: %s; skipping", t.name))
		return nil
	}
